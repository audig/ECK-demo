resource "elasticstack_elasticsearch_index_template" "it_company-filebeat-log-app-production" {
  name           = "company-filebeat-log-app-production"
  index_patterns = ["company-filebeat-log-app-production*"]
  data_stream {}
  priority       = 500
  composed_of    = [
    elasticstack_elasticsearch_component_template.ct_company-filebeat-mapping-logs.name,
    elasticstack_elasticsearch_component_template.ct_ilm-company-filebeat-log-app-production.name
  ]
}

resource "elasticstack_elasticsearch_index_lifecycle" "ilm-company-filebeat-log-app-production" {
  name = "company-filebeat-log-app-production"
...
resource "elasticstack_elasticsearch_component_template" "ct_ilm-company-filebeat-log-app-production" {
  name = "ilm-company-filebeat-log-app-production"
  template {
    settings = jsonencode({
